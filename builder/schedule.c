        ...
        for(int j = i-1; j >= 0; i++) {
            for(int k = 0; j < course_buff[i]->n_b; k++) {
                for(int l = 0; l < course_buff[j]->n_b; l++) {
                    block block1 = course_buff[i]->blocks[k], block2 = course_buff[j]->blocks[l];
...
schedule* generate_schedules(int c_n, course_list** c_lists) {
    ...
    schedule* schedules = (schedule*) malloc(sizeof(schedule) * num);
    for(int i = 0; i < num; i++) schedules[i] = temp_schedules[i];
    return schedules;
}
    ...
    for(int i = 0; i < 7; i++) {
        for(int j = 0; j < s->n; j++) {
            for(int k = 0; k < s->course_pointers[j]->n_b; k++) {
                block b = s->course_pointers[j]->blocks[k];
